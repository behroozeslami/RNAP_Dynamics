function Analyze_1mM_ke1_SingleTraces(data_name, subfolder_name, selected_trace_indices) 

mainfoldername = 'Select_AF_7.5pN_1mM_NTP';

grid_name = 'ML_grid_1mM';

save_folder = 'Analysis_1mM_SingleTraces_ke1';
...
ke1_bins = unique(grid(:,1));

delta_ke1 = ke1_bins(2)-ke1_bins(1);

[Best_ke1, Mean_ke1, LB_ke1, UB_ke1, std_ke1] = Conf_int(ke1_bins,Ave_Prob_grid,alpha);

Fit_dat = [Mean_ke1, LB_ke1, UB_ke1, std_ke1/sqrt(length(selected_trace_indices))]';

str_title = strrep(data_name, '_', ' - ');

PL1 = figure(1);

bar(ke1_bins,Ave_Prob_grid,'r')

hold on

line([LB_ke1-0.5*delta_ke1,LB_ke1-0.5*delta_ke1],ylim,'LineWidth', 2, 'Color', 'b')

line([UB_ke1+0.5*delta_ke1,UB_ke1+0.5*delta_ke1],ylim,'LineWidth', 2, 'Color', 'b')

line([Mean_ke1,Mean_ke1],ylim,'LineWidth', 2, 'Color', 'g')

hold off

xlabel('Optimal ke1 (bp/s)')

ylabel('Frequency')

title(str_title)

str_ke1_conf_int = [save_subfolder slash Param_name{1} '_' 'Confidence_Interval' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];

saveas(PL1,[str_ke1_conf_int,'.fig'], 'fig')

saveas(PL1,[str_ke1_conf_int,'.jpg'], 'jpg')
...
xlswrite([str_Best_Param_hist, '.xlsx'],ke1_bins,'Sheet1','A1');
