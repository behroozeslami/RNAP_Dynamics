function Analyze_1mM_ke2_SingleTraces(data_name, subfolder_name, selected_trace_indices) 

mainfoldername = 'Select_AF_7.5pN_1mM_NTP';

grid_name = 'ML_grid_1mM';

save_folder = 'Analysis_1mM_SingleTraces_ke2';
...
[Best_ke2, Mean_ke2, LB_ke2, UB_ke2, std_ke2] = Conf_int(ke2_bins,Ave_Prob_grid,alpha);

Fit_dat = [Mean_ke2, LB_ke2, UB_ke2, std_ke2/sqrt(length(selected_trace_indices))]';

str_title = strrep(data_name, '_', ' - ');

PL1 = figure(1);

bar(ke2_bins,Ave_Prob_grid,'r')
...
xlabel('Optimal ke2 (bp/s)')

ylabel('Frequency')

title(str_title)

str_ke2_conf_int = [save_subfolder slash Param_name{1} '_' 'Confidence_Interval' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];
...
xlswrite([str_Best_Param_hist, '.xlsx'],ke2_bins,'Sheet1','A1');
