function Analyze_1mM_ke3_SingleTraces(data_name, subfolder_name, selected_trace_indices) 

mainfoldername = 'Select_AF_7.5pN_1mM_NTP';

grid_name = 'ML_grid_1mM';

save_folder = 'Analysis_1mM_SingleTraces_ke3';
...
[Best_ke3, Mean_ke3, LB_ke3, UB_ke3, std_ke3] = Conf_int(ke3_bins,Ave_Prob_grid,alpha);

Fit_dat = [Mean_ke3, LB_ke3, UB_ke3, std_ke3/sqrt(length(selected_trace_indices))]';

str_title = strrep(data_name, '_', ' - ');

PL1 = figure(1);

bar(ke3_bins,Ave_Prob_grid,'r')
...
xlabel('Optimal ke3 (bp/s)')

ylabel('Frequency')

title(str_title)

str_ke3_conf_int = [save_subfolder slash Param_name{1} '_' 'Confidence_Interval' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];
...
xlswrite([str_Best_Param_hist, '.xlsx'],ke3_bins,'Sheet1','A1');
