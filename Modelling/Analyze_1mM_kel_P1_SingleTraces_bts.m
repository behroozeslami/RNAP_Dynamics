function Analyze_1mM_kel_P1_SingleTraces_bts(data_name, subfolder_name, selected_trace_indices) 
...
save_folder = 'Analysis_1mM_SingleTraces_kel-P1';
...
[Param_dat, txt] = xlsread('Parameters/General_Model_Parameters_Select_AF_7.5pN_1mM_NTP.xlsx');
...
P1_bins = unique(grid(:,2));

delta_kel = kel_bins(2)-kel_bins(1);

delta_P1 = P1_bins(2)-P1_bins(1);

P1_Prob_grid = cell2mat(arrayfun(@(P1) sum(Ave_Prob_grid(grid(:,2)==P1,1)), P1_bins,'UniformOutput',0));

kel_Prob_grid = cell2mat(arrayfun(@(kel) sum(Ave_Prob_grid(grid(:,1)==kel,1)), kel_bins,'UniformOutput',0));

[Best_P1, Mean_P1, LB_P1, UB_P1] = Conf_int(P1_bins,P1_Prob_grid,alpha);

[Best_kel, Mean_kel, LB_kel, UB_kel] = Conf_int(kel_bins,kel_Prob_grid,alpha);

Fit_dat = [[Mean_kel, LB_kel, UB_kel]', [Mean_P1, LB_P1, UB_P1]'];
...
bar(P1_bins,P1_Prob_grid,'r')

hold on

line([LB_P1-0.5*delta_P1,LB_P1-0.5*delta_P1],ylim,'LineWidth', 2, 'Color', 'b')

line([UB_P1+0.5*delta_P1,UB_P1+0.5*delta_P1],ylim,'LineWidth', 2, 'Color', 'b')

line([Mean_P1,Mean_P1],ylim,'LineWidth', 2, 'Color', 'g')
...
Ave_Prob_mat = zeros([length(P1_bins), length(kel_bins)]);

for n=1:length(P1_bins)
    ...
    for m=1:length(kel_bins)
        ...
        Ave_Prob_mat(n,m) = Ave_Prob_grid(((grid(:,2)==P1)&(grid(:,1)==kel)),1);
...
surf(kel_bins, P1_bins, Ave_Prob_mat);
...
[Min,Min_Ind] = min((Mean_kel-grid(:,1)).^2 + (Mean_P1-grid(:,2)).^2);
...
str_P1_conf_int = [save_subfolder slash Param_name_1{1} '_' 'Confidence_Interval' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];

saveas(PL1,[str_kel_conf_int,'.fig'], 'fig')

saveas(PL1,[str_kel_conf_int,'.jpg'], 'jpg')

saveas(PL2,[str_P1_conf_int,'.fig'], 'fig')

saveas(PL2,[str_P1_conf_int,'.jpg'], 'jpg')
...
xlswrite([str_Best_Param_hist, '.xlsx'],P1_bins,'Sheet1','A3');
...
row_header(1:3,1) = {'Mean', '1-sigma LB', '1-sigma UB'};
