function Fit_dat = Analyze_1mM_kel_P_NewSel_SingleTraces_bts(data_name, subfolder_name, selected_trace_indices) 
...
selected_trace_indices = selected_trace_indices(~ismember(selected_trace_indices, blacklist));
...
uniform_weigts = 1/length(Unique_dat_length)*ones([1, length(Unique_dat_length)]);

Prob = Prob_grid*transpose(uniform_weigts);
...
P_Prob_grid = cell2mat(arrayfun(@(P) sum(Prob(grid(:,2)==P,1)), P_bins,'UniformOutput',0));

kel_Prob_grid = cell2mat(arrayfun(@(kel) sum(Prob(grid(:,1)==kel,1)), kel_bins,'UniformOutput',0));
...
for n=1:length(P_bins)
    ...
    for m=1:length(kel_bins)
        ...
        Ave_Prob_mat(n,m) = Prob(((grid(:,2)==P)&(grid(:,1)==kel)),1);
...
kel_hist = [kel_bins,kel_Prob_grid];

P_hist = [P_bins,P_Prob_grid];

col_header = {'bin centers', 'hist values'};

str_kel_hist = [save_subfolder slash Param_name_1{1} '_' 'histogram' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];

str_P_hist = [save_subfolder slash Param_name_2{1} '_' 'histogram' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];

xlswrite([str_kel_hist, '.xlsx'],kel_hist,'Sheet1','A2');

xlswrite([str_kel_hist, '.xlsx'],col_header,'Sheet1','A1');

xlswrite([str_P_hist, '.xlsx'],P_hist,'Sheet1','A2');

xlswrite([str_P_hist, '.xlsx'],col_header,'Sheet1','A1');
