function Analyze_1mM_kp3_SingleTraces(data_name, subfolder_name, selected_trace_indices) 

mainfoldername = 'Select_AF_7.5pN_1mM_NTP';

grid_name = 'ML_grid_1mM';

save_folder = 'Analysis_1mM_SingleTraces_kp3';
...
kp3_bins = unique(grid(:,1));

delta_kp3 = kp3_bins(2)-kp3_bins(1);

[Best_kp3, Mean_kp3, LB_kp3, UB_kp3, std_kp3] = Conf_int(kp3_bins,Ave_Prob_grid,alpha);

Fit_dat = [Mean_kp3, LB_kp3, UB_kp3, std_kp3/sqrt(length(selected_trace_indices))]';

str_title = strrep(data_name, '_', ' - ');

PL1 = figure(1);

bar(kp3_bins,Ave_Prob_grid,'r')
...
xlabel('Optimal kp3 (bp/s)')

ylabel('Frequency')

title(str_title)

str_kp3_conf_int = [save_subfolder slash Param_name{1} '_' 'Confidence_Interval' '_' data_name '_' 'Ts=' num2str((2*W+1)*t0) 's' '_' 'Nw=' num2str(Nw) '_' 'limits=' num2str(t_strt) '-' num2str(t_end)];

saveas(PL1,[str_kp3_conf_int,'.fig'], 'fig')

saveas(PL1,[str_kp3_conf_int,'.jpg'], 'jpg')
...
xlswrite([str_Best_Param_hist, '.xlsx'],kp3_bins,'Sheet1','A1');
