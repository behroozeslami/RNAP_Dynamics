...
MM = fittype('Vmax*(1+8.5./x)./(1+K./x)');

[MM_fit,gof,fitinfo] = fit(x,y,MM,'Startpoint',[10, 25]);

VM = MM_fit.Vmax;

KM = MM_fit.K;

kel_predict = arrayfun(@(C) VM*(1+8.5/C)/(1+KM/C), 0.5:0.01:1000);

PL = figure();

semilogx(0.5:0.01:1000, kel_predict, 'k', 'LineWidth', 2)
