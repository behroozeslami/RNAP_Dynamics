...
mainfoldername = 'Pause free Velocity';
...
trace_folder = [output_foldername slash 'traces' '_' 'Ts=' num2str(2*W+1) '_' 'Pause_threshold=' num2str(Pause_threshold) 's'];
...
for i=1:L
    ...
    plot(t_array, x_array, 'color', [0.5,0.5,0.5])
    
    hold on
    
    PL = plot(t_array, xs_array, 'g', 'LineWidth', 2)
    ...
    for j=1:length(chunk_start)
        ...
        t_chunk = t_array(chunk_start(j):chunk_end(j));
        
        plot(t_chunk, x_chunk, 'r', 'LineWidth', 2)
    end
