...
data_name = 'GreA_cotoff=0.1';

file_path = ['Modelling' slash output_foldername slash data_name '.txt'];
...
str_DWT = [DWT_folder slash 'DWT_pdf_Combined' '_' data_name '_' 'Ts='  num2str(2*W+1) '_' 'Nw=' num2str(Nw) '_' 'l0_range=' num2str(l0_min) '-' num2str(l0_max) '_' 'Seidel_range=' num2str(Seidel_min) '-' num2str(Seidel_max)];

DWT_array = [];

selected_trace_number = load(file_path);
...
PL = errorbar(bar_pos,DWT_hist,Err,'bO','MarkerSize',6, 'MarkerFaceColor', 'b');

set(ax,'XScale','log','YScale','log')

str_title = [strrep(data_name,'_','-') '  ' 'Ts = ' ' ' num2str(2*W+1) ' ' 'Nw = ' num2str(Nw) ' ' 'l0 = ' num2str(l0_min) '-' num2str(l0_max) ' ' 'Seidel = ' num2str(Seidel_min) '-' num2str(Seidel_max)];
