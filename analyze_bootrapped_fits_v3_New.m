...
mainfoldername = 'Select_AF_7.5pN_1mM_NTP_same';
...
file_to_correct_kel_P = './Modelling/Select_AF_7.5pN_1mM_NTP_same/Analysis_1mM_SingleTraces_kel-P/1mM_Select/Fit_outcome_bts_1mM_Select_kel-P_Ts=1s_Nw=10_limits=0.05-10000.xlsx';

if exist(file_to_correct_kel_P, 'file') == 2
    
    correct_dat = xlsread(file_to_correct_kel_P);
    
    All_micromodel_params(:,1) = correct_dat(:,1);
    
    All_micromodel_params(:,2) = correct_dat(:,2);
    
end

dat_micro = [mean(All_micromodel_params); std(All_micromodel_params)];

output_file_str_2 = [param_foldername slash 'Micro_Model_1_Parameters' '_' output_foldername '_' 'withBts' '.xlsx'];
...
xlswrite(output_file_str_2,All_micromodel_params,'Sheet2','A2');

xlswrite(output_file_str_2,col_header_2,'Sheet2','A1');
